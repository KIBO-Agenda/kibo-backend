...
async def trigger_manual_reminder(
    ...
    try:
        ...
        if appointment.status != AppointmentStatus.PENDING:
            logger.warning(f"[TEST_REMINDER] Appointment {appointment_id} is not PENDING (status: {appointment.status})")
        ...
        if not tenant.whatsapp_instance_id:
            raise HTTPException(
                status_code=400,
                detail="Tenant does not have WhatsApp instance configured"
            )
        ...
        if not remote_jid:
            logger.warning(
                f"[TEST_REMINDER] Could not extract remoteJid from Evolution response",
